...
# Extractions the TOOL never reads, kept for whoever is reverse-engineering.
#
# data/ is shipped WHOLE - the App .csproj globs data\*.json and data\*.tsv into the release,
# deliberately and with a comment saying why: a narrower glob once shipped a build in which
# every unique was nameless. So the glob stays broad and the rule is on the directory instead:
# data/ holds what the tool LOADS, nothing else. Six of these maps were read by nothing at all
# and rode along in every download for it, so they live here now - still in the repository,
# still regenerated by build-item-names, no longer 1.5 MB of every release.
EXTRACTED_DIR = os.path.join(SCRIPT_DIR, "extracted")


def extracted(name):
    """Path in tools/extracted/, making sure the directory is there to write into."""
...
def extract_mods(csv_dir=None, output_tsv=None):
    csv_dir = csv_dir or DEFAULT_CSV_DIR
    output_tsv = output_tsv or extracted("mod_name_map.tsv")
...
def extract_monsters(csv_dir=None, output_tsv=None):
    csv_dir = csv_dir or DEFAULT_CSV_DIR
    output_tsv = output_tsv or extracted("monster_name_map.tsv")
...
def build_item_names(csv_dir=None):
    ...
    mod_tsv = extracted("mod_name_map.tsv")
    ...
    base_tsv = extracted("base_item_name_map.tsv")
    ...
    unique_tsv = extracted("unique_name_map.tsv")
    ...
    unique_item_tsv = extracted("unique_item_name_map.tsv")
    ...
    skill_tsv = extracted("skill_name_map.tsv")
...
def extract_worldareas(game_dir=None, output_tsv=None, maps_only=False):
    ...
    output_tsv = output_tsv or extracted("world_area_name_map.tsv")
...
def extract_inventories(game_dir=None, output_tsv=None):
    game_dir = game_dir or DEFAULT_GAME_DIR
    output_tsv = output_tsv or extracted("inventory_type_map.tsv")
